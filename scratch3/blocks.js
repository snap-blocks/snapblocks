...
export class IconView {
  ...
  static get icons() {
    return {
      ...
      arrowUp: {},
      arrowUpOutline: { fillAttribute: "stroke" },
      ...
      arrowRight: {},
      arrowRightOutline: { fillAttribute: "stroke" },
...
class ScriptView {
  ...
  draw(options, inside) {
    ...
    for (const block of this.blocks) {
      const x = inside ? 0 : 2
      if (options.zebraColoring && block.isBlock) {
        if (
          inside &&
          !this.isZebra &&
    ...
    this.options = {
      ...
      commentWidth:
        (options.commentWidth > 0
          ? Math.max(100, options.commentWidth)
          : options.commentWidth) || 200,
    }
  }
